...
void CDUIRenderEngine::DrawLine(HDC hDC, const CDUIRect &rcItem, int nSize, DWORD dwPenColor, int nStyle /*= PS_SOLID*/)
{
	ASSERT(::GetObjectType(hDC) == OBJ_DC || ::GetObjectType(hDC) == OBJ_MEMDC);
...
void CDUIRenderEngine::DrawRect(HDC hDC, const CDUIRect &rcItem, int nSize, DWORD dwPenColor)
{
	ASSERT(::GetObjectType(hDC) == OBJ_DC || ::GetObjectType(hDC) == OBJ_MEMDC);
...
void CDUIRenderEngine::DrawRound(HDC hDC, const CDUIRect &rcItem, int nXRound, int nYRound, int nSize, DWORD dwPenColor)
{
	ASSERT(::GetObjectType(hDC) == OBJ_DC || ::GetObjectType(hDC) == OBJ_MEMDC);
	...
	Gp.DrawArc(&Pen, rcItem.right - nXRound * 2 - nSize, rcItem.top, nXRound * 2, nYRound * 2, 270, 90); // 右上圆角
	Gp.DrawArc(&Pen, rcItem.right - nXRound * 2 - nSize, rcItem.bottom - nYRound * 2 - nSize, nXRound * 2, nYRound * 2, 0, 90); // 右下圆角
	Gp.DrawArc(&Pen, rcItem.left, rcItem.bottom - nYRound * 2 - nSize, nXRound * 2, nYRound * 2, 90, 90); // 左下圆角

	return;
}

void CDUIRenderEngine::DrawRoundRect(HDC hDC, const CDUIRect &rcItem, int nXRound, int nYRound, int nSize, DWORD dwPenColor)
{
	ASSERT(::GetObjectType(hDC) == OBJ_DC || ::GetObjectType(hDC) == OBJ_MEMDC);
	...
	//gid+
	{
		...
		Path.AddLine(rcItem.left + nXRound, rcItem.top, rcItem.right - nXRound, rcItem.top); //顶部横线
		Path.AddArc(rcItem.right - nXRound * 2, rcItem.top, nXRound * 2, nYRound * 2, 270, 90); //右上圆角
		Path.AddLine(rcItem.right, rcItem.top + nYRound, rcItem.right, rcItem.bottom - nYRound); //右侧竖线
		Path.AddArc(rcItem.right - nXRound * 2, rcItem.bottom - nYRound * 2, nXRound * 2, nYRound * 2, 0, 90); //右下圆角
		Path.AddLine(rcItem.right - nXRound, rcItem.bottom, rcItem.left + nXRound, rcItem.bottom); //底部横线
		Path.AddArc(rcItem.left, rcItem.bottom - nYRound * 2, nXRound * 2, nYRound * 2, 90, 90); //左下圆角
		Path.AddLine(rcItem.left, rcItem.bottom - nYRound, rcItem.left, rcItem.top + nYRound); //左侧竖线
		Path.AddArc(rcItem.left, rcItem.top, nXRound * 2, nYRound * 2, 180, 90); //左上圆角
...
void CDUIRenderEngine::FillRoundRect(HDC hDC, const CDUIRect &rcItem, int nXRound, int nYRound, DWORD dwColor)
{
	Gdiplus::Graphics Gp(hDC);
	Gp.SetSmoothingMode(Gdiplus::SmoothingModeHighQuality);
	Gp.SetInterpolationMode(Gdiplus::InterpolationModeHighQualityBicubic);
