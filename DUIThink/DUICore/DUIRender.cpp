...
void CDUIRenderEngine::DrawAnimateImage(HDC hDC, Gdiplus::Bitmap *pBmpAnimate, const CDUIRect &rcItem, int nFrameCur, const CDUISize &szRound)
{
	...
	if (szRound.cx > 0 || szRound.cy > 0)
	{
		...
		Path.AddLine(rcItem.left + szRound.cx, rcItem.top, rcItem.right - szRound.cx, rcItem.top); //顶部横线
		Path.AddArc(rcItem.right - szRound.cx * 2, rcItem.top, szRound.cx * 2, szRound.cy * 2, 270, 90); //右上圆角
		Path.AddLine(rcItem.right, rcItem.top + szRound.cy, rcItem.right, rcItem.bottom - szRound.cy); //右侧竖线
		Path.AddArc(rcItem.right - szRound.cx * 2, rcItem.bottom - szRound.cy * 2, szRound.cx * 2, szRound.cy * 2, 0, 90); //右下圆角
		Path.AddLine(rcItem.right - szRound.cx, rcItem.bottom, rcItem.left + szRound.cx, rcItem.bottom); //底部横线
		Path.AddArc(rcItem.left, rcItem.bottom - szRound.cy * 2, szRound.cx * 2, szRound.cy * 2, 90, 90); //左下圆角
		Path.AddLine(rcItem.left, rcItem.bottom - szRound.cy, rcItem.left, rcItem.top + szRound.cy); //左侧竖线
		Path.AddArc(rcItem.left, rcItem.top, szRound.cx * 2, szRound.cy * 2, 180, 90); //左上圆角
...
void CDUIRenderEngine::DrawRound(HDC hDC, const CDUIRect &rcItem, const CDUISize &szRound, int nLineSize, DWORD dwPenColor)
{
	...
	Gdiplus::Pen Pen(Gdiplus::Color(dwPenColor), nLineSize);
	Pen.SetAlignment(Gdiplus::PenAlignmentInset);
	Gp.DrawArc(&Pen, rcItem.left, rcItem.top, szRound.cx * 2, szRound.cy * 2, 180, 90); // 左上圆角
	Gp.DrawArc(&Pen, rcItem.right - szRound.cx * 2 - nLineSize, rcItem.top, szRound.cx * 2, szRound.cy * 2, 270, 90); // 右上圆角
	Gp.DrawArc(&Pen, rcItem.right - szRound.cx * 2 - nLineSize, rcItem.bottom - szRound.cy * 2 - nLineSize, szRound.cx * 2, szRound.cy * 2, 0, 90); // 右下圆角
	Gp.DrawArc(&Pen, rcItem.left, rcItem.bottom - szRound.cy * 2 - nLineSize, szRound.cx * 2, szRound.cy * 2, 90, 90); // 左下圆角
...
void CDUIRenderEngine::DrawRoundRect(HDC hDC, const CDUIRect &rcItem, const CDUISize &szRound, int nLineSize, DWORD dwPenColor, CDUISize szBreakTop)
{
	...
	//gid+
	{
		...
		Path.AddArc(rcDraw.right - szRound.cx * 2 - nLineSize, rcDraw.top, szRound.cx * 2, szRound.cy * 2, 270, 90); //右上圆角
		Path.AddLine(rcDraw.right - nLineSize, rcDraw.top + szRound.cy, rcDraw.right - nLineSize, rcDraw.bottom - szRound.cy); //右侧竖线
		Path.AddArc(rcDraw.right - szRound.cx * 2 - nLineSize, rcDraw.bottom - szRound.cy * 2 - nLineSize, szRound.cx * 2, szRound.cy * 2, 0, 90); //右下圆角
		Path.AddLine(rcDraw.right - szRound.cx, rcDraw.bottom - nLineSize, rcDraw.left + szRound.cx, rcDraw.bottom - nLineSize); //底部横线
		Path.AddArc(rcDraw.left, rcDraw.bottom - szRound.cy * 2 - nLineSize, szRound.cx * 2, szRound.cy * 2, 90, 90); //左下圆角
		Path.AddLine(rcDraw.left, rcDraw.bottom - szRound.cy, rcDraw.left, rcDraw.top + szRound.cy); //左侧竖线
		Path.AddArc(rcDraw.left, rcDraw.top, szRound.cx * 2, szRound.cy * 2, 180, 90); //左上圆角
...
void CDUIRenderEngine::FillRoundRect(HDC hDC, const CDUIRect &rcItem, const CDUISize &szRound, int nLineSize, DWORD dwColor, DWORD dwColorGradient)
{
	...
	//path
	Gdiplus::GraphicsPath Path;
	Path.AddLine(rcDraw.left + szRound.cx, rcDraw.top, rcDraw.right - szRound.cx, rcDraw.top); //顶部横线
	Path.AddArc(rcDraw.right - szRound.cx * 2 - nLineSize, rcDraw.top, szRound.cx * 2, szRound.cy * 2, 270, 90); //右上圆角
	Path.AddLine(rcDraw.right - nLineSize, rcDraw.top + szRound.cy, rcDraw.right - nLineSize, rcDraw.bottom - szRound.cy); //右侧竖线
	Path.AddArc(rcDraw.right - szRound.cx * 2 - nLineSize, rcDraw.bottom - szRound.cy * 2 - nLineSize, szRound.cx * 2, szRound.cy * 2, 0, 90); //右下圆角
	Path.AddLine(rcDraw.right - szRound.cx, rcDraw.bottom - nLineSize, rcDraw.left + szRound.cx, rcDraw.bottom - nLineSize); //底部横线
